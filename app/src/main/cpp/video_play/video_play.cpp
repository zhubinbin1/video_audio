...
void init() {
    ...
    //得到播放总时间
    if (pFormatCtx2->duration != AV_NOPTS_VALUE) {
        duration = pFormatCtx2->duration;//微秒
    }
}
...
void seekTo(int mesc) {
   ...
   /* if (av_seek_frame(pFormatCtx, -1,  mesc * AV_TIME_BASE, AVSEEK_FLAG_BACKWARD) < 0) {
    ...
    av_seek_frame(pFormatCtx2, ffmpegVideo->index, (int64_t) (mesc /av_q2d(ffmpegVideo->time_base)), AVSEEK_FLAG_BACKWARD);
    av_seek_frame(pFormatCtx2, ffmpegMusic->index, (int64_t) (mesc /av_q2d(ffmpegMusic->time_base)), AVSEEK_FLAG_BACKWARD);
...
void *begin(void *args) {
    ...
    //找到视频流和音频流
    for (int i = 0; i < pFormatCtx2->nb_streams; ++i) {
        //获取解码器
        AVCodecContext *avCodecContext = pFormatCtx2->streams[i]->codec;
        ...
        if (pFormatCtx2->streams[i]->codec->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            ffmpegVideo->time_base = pFormatCtx2->streams[i]->time_base;
            if (window) {
            ...
            ffmpegMusic->time_base = pFormatCtx2->streams[i]->time_base;
        }
    }
//开启播放
    ...
    //seekTo(0);
    //解码packet,并压入队列中
    packet2 = (AVPacket *) av_mallocz(sizeof(AVPacket));
    //跳转到某一个特定的帧上面播放
    int ret;
    while (isPlay) {
        //
        ret = av_read_frame(pFormatCtx2, packet2);
        if (ret == 0) {
            if (ffmpegVideo && ffmpegVideo->isPlay && packet2->stream_index == ffmpegVideo->index
               ) {
                //将视频packet压入队列
                ffmpegVideo->put(packet2);
            } else if (ffmpegMusic && ffmpegMusic->isPlay &&
                       packet2->stream_index == ffmpegMusic->index) {
                ffmpegMusic->put(packet2);
            }
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_binbin_myapplication_ffmpegvideoplay_Play_display(JNIEnv *env, jobject instance, jobject surface) {
    //得到界面
    if (window) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_binbin_myapplication_ffmpegvideoplay_Play_release(JNIEnv *env, jobject instance) {
    //释放资源
    if (isPlay) {
        isPlay = 0;
...
JNIEXPORT void JNICALL
Java_com_example_binbin_myapplication_ffmpegvideoplay_Play_stop(JNIEnv *env, jobject instance) {
    //点击暂停按钮
    ffmpegMusic->pause();
    ffmpegVideo->pause();
...
Java_com_example_binbin_myapplication_ffmpegvideoplay_Play_getTotalTime(JNIEnv *env, jobject instance) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_binbin_myapplication_ffmpegvideoplay_Play_seekTo(JNIEnv *env, jobject instance, jint msec) {
seekTo(msec/1000);
}
extern "C"
JNIEXPORT void JNICALL
Java_com_example_binbin_myapplication_ffmpegvideoplay_Play_stepBack(JNIEnv *env, jobject instance) {

}
extern "C"
JNIEXPORT void JNICALL
Java_com_example_binbin_myapplication_ffmpegvideoplay_Play_stepUp(JNIEnv *env, jobject instance) {
    //点击快进按钮
    seekTo(5);
