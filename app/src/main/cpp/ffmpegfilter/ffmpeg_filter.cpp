...
const char *filters_descr2 = "lutyuv='u=128:v=128'";
...
Java_com_example_binbin_myapplication_ffmpegandroidavfilter_MainActivity_play
    ...
    filter_graph2 = avfilter_graph_alloc();
    ...
    ret = avfilter_graph_create_filter(&buffersrc_ctx2, buffersrc, "in",
                                       args, NULL, filter_graph2);
    if (ret < 0) {
    ...
    ret = avfilter_graph_create_filter(&buffersink_ctx2, buffersink, "out",
                                       NULL, buffersink_params, filter_graph2);
    ...
    outputs->filter_ctx = buffersrc_ctx2;
    outputs->pad_idx    = 0;
    outputs->next       = NULL;

    inputs->name       = av_strdup("out");
    inputs->filter_ctx = buffersink_ctx2;
    inputs->pad_idx    = 0;
    inputs->next       = NULL;

    // avfilter_link(buffersrc_ctx, 0, buffersink_ctx, 0);

    if ((ret = avfilter_graph_parse_ptr(filter_graph2, filters_descr2,
                                        &inputs, &outputs, NULL)) < 0) {
        LOGD("Cannot avfilter_graph_parse_ptr\n");
        return ret;
    }

    if ((ret = avfilter_graph_config(filter_graph2, NULL)) < 0) {
    ...
    while (av_read_frame(pFormatCtx, &packet) >= 0) {
        ...
        if (packet.stream_index == videoStream) {
            ...
            if (frameFinished) {
                ...
                if (av_buffersrc_add_frame(buffersrc_ctx2, pFrame) < 0) {
                    LOGD("Could not av_buffersrc_add_frame");
                    break;
                }

                ret = av_buffersink_get_frame(buffersink_ctx2, pFrame);
                if (ret < 0) {
    ...
    avfilter_graph_free(&filter_graph2); //added by ws for avfilter
    // Close the codecs
